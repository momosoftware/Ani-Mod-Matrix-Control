...
class idol:
    def __init__(self, matrixCom, matrixType,musicSource,dtvSource):
        ...
        self.dtvSource = dtvSource
    
    def _buildCommand(self,source,targets):
        # Takets a source (input) and a list of targets (outputs) and generates a command for them for whichever matrix you're using
        if matrixType == "animod":
            commandBase = str(source) + "B"
            for target in targets:
                commandBase += str(target) + ", "
            command = commandBase[:-2] + "."
        elif matrixType == "wolf":
    ...
    def _sendSerial(self,com,cmdString):
        try:
            com.write(bytes(cmdString, 'UTF-8'))
    ...
    def standardScene(self):
        # music dtv1 music dtv2 music dtv3 music dtv4 music (etc)
        # need to build music and dtv output lists from total number of outputs
        # need to build dtvSourceList from config
        logger.debug('====================')
        logger.debug('Set standard AV config odd')
        musicOutputList = [1,3,5,7,9]
        dtvOutputList = [2,4,6,8]
        logger.debug("Music outputs: " + str(musicOutputList))
        logger.debug("DTV outputs: " + str(dtvOutputList))
        musicOutCommand = self._buildCommand(self.musicSource, musicOutputList)
        dtvOutCommand = self._buildCommand(self.dtvSource, dtvOutputList)
...
#
#
#       EVERYTHING BELOW THIS POINT NEEDS REWRITTEN 
#       FOR WEB/FLASK AND TO SUPPORT MULTIPLE MATRICIES
#
#
#
#
